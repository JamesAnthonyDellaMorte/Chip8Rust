...
impl cpu {
    ...
    pub fn init_memory(&mut self, path: String) {
        ...
        self.memory[0..self.hexSprites.len()].clone_from_slice(&self.hexSprites[..]);
        self.memory[0x200..(rom.len() + 0x200)].clone_from_slice(&rom[..]);
    ...
    pub fn LDKEY(&mut self, x: u8) {
        ...
        'outer: loop {
            for ele in &self.keycode {
                if ele.is_pressed() {
                    self.V[x as usize] =
                        self.keycode.iter().position(|&r| r == *ele).unwrap() as u8;
                    break 'outer;
                }
            }
        }
